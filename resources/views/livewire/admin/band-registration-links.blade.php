        ...
        <div class="space-y-6">
            ...
            <div class="rounded-lg bg-white p-6 shadow">
                <div class="mb-6 flex flex-col space-y-4 lg:flex-row lg:items-center lg:justify-between lg:space-y-0">
                    ...
                    <div class="flex space-x-3">
                        <select wire:model.live="filterStatus"
                            ...
                            <option value="token_only">Nur Token (ohne Email)</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="mb-6 flex flex-wrap gap-3">
                    ...
                    {{-- NEU: Nur Links generieren --}}
                    <button wire:click="generateTokensOnly" @if (empty($selectedBands)) disabled @endif
                        class="inline-flex items-center rounded-md bg-green-600 px-4 py-2 text-white hover:bg-green-700 disabled:cursor-not-allowed disabled:opacity-50">
                        <svg class="mr-2 h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1">
                            </path>
                        </svg>
                        Nur Links generieren ({{ count($selectedBands) }})
                    </button>

                    {{-- NEU: Token erneuern --}}
                    <button wire:click="regenerateTokens" @if (empty($selectedBands)) disabled @endif
                        onclick="return confirm('Bestehende Tokens werden überschrieben. Fortfahren?')"
                        class="inline-flex items-center rounded-md bg-orange-600 px-4 py-2 text-white hover:bg-orange-700 disabled:cursor-not-allowed disabled:opacity-50">
                        <svg class="mr-2 h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15">
                            </path>
                        </svg>
                        Tokens erneuern ({{ count($selectedBands) }})
                    </button>
                    {{-- NEU: Alle Links kopieren --}}
                    @if (count($selectedBands) > 0)
                        <button wire:click="copyAllLinks"
                            class="inline-flex items-center rounded-md bg-gray-600 px-4 py-2 text-white hover:bg-gray-700">
                            <svg class="mr-2 h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            @forelse($bands as $band)
                                <tr class="hover:bg-gray-50" wire:key="band-{{ $band->id }}">
                                    <td class="whitespace-nowrap px-6 py-4">
                                        <input type="checkbox" wire:model="selectedBands"
                                            value="{{ $band->id }}"
                                            class="rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                                    </td>
                                    <td class="whitespace-nowrap px-6 py-4">
                                        <div>
                                            <div class="text-sm font-medium text-gray-900">{{ $band->band_name }}
                                            </div>
                                            <div class="text-sm text-gray-500">
                                                {{ $band->stage->name ?? 'Keine Bühne' }}</div>
                                        </div>
                                    </td>
                                    <td class="whitespace-nowrap px-6 py-4">
                                        @if ($band->manager_full_name)
                                            <div class="text-sm">
                                                <div class="font-medium text-gray-900">{{ $band->manager_full_name }}
                                                </div>
                                            ...
                                            <div class="text-sm">
                                                ...
                                                @if ($band->registration_reminder_sent_at)
                                                    <div class="text-xs text-orange-600">🔔 Erinnerung:
                                                        {{ $band->registration_reminder_sent_at->format('d.m.Y') }}
                                                    </div>
                                        ...
                                        @if ($band->registration_completed)
                                            <div class="text-sm">
                                                ...
                                                @if ($band->travel_party)
                                                    <div class="text-gray-500">{{ $band->travel_party }} Mitglieder
                                                    </div>
                                                @endif
                                            </div>
                                    ...
                                    <!-- In der Tabelle bei den Aktionen -->
                                    <td class="whitespace-nowrap px-6 py-4 text-right text-sm font-medium">
                                        <div class="flex items-center space-x-2">
                                            ...
                                            @if ($band->registration_token && !$band->registration_completed)
                                                ...
                                                @endif
                                                {{-- NEU: Link generieren ohne Email --}}
                                            @elseif(!$band->registration_token)
                                                <button wire:click="generateSingleToken({{ $band->id }})"
                                                    class="text-xs text-green-600 hover:text-green-900">
                                                    🔗 Link generieren
                                                </button>
                                            @endif
                                            ...
                                            @if ($band->registration_token)
                                                ...
                                                {{-- NEU: Token erneuern --}}
                                                <button wire:click="regenerateSingleToken({{ $band->id }})"
                                                    onclick="return confirm('Token wirklich erneuern?')"
...
<script>
    document.addEventListener('livewire:init', function() {
        Livewire.on('copy-to-clipboard', function(event) {
            ...
            function fallbackCopyTextToClipboard(text) {
                ...
                try {
                    ...
                    showToast('Kopieren wird nicht unterstützt', 'error');
                }
            ...
            if (navigator.clipboard && window.isSecureContext) {
                ...
                // Fallback für unsichere Kontexte oder ältere Browser
                fallbackCopyTextToClipboard(event.text);
            }
        });
        ...
        function showToast(message, type = 'success') {
            ...
            toast.className =
                `fixed top-4 right-4 ${bgColor} text-white px-4 py-2 rounded-md shadow-lg z-50 transition-opacity duration-300`;
